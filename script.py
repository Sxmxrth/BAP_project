        ...
        for host in nm.all_hosts():
            try:
                ...
                host_data = {
                    ...
                    "Hostname": (
                        host_info["hostnames"][0]["name"]
                        if host_info["hostnames"]
                        else ""
                    ),
                    "MAC Address": (
                        host_info["addresses"]["mac"]
                        if "mac" in host_info["addresses"]
                        else ""
                    ),
                    "Vendor": (
                        host_info["vendor"][host_info["addresses"]["mac"]]
                        if "mac" in host_info["addresses"] and host_info["vendor"]
                        else "Unknown"
                    ),
                    "OS": (
                        host_info["osmatch"][0]["name"] if host_info["osmatch"] else ""
                    ),
